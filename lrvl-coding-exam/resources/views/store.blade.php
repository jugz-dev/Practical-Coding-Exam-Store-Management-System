        ...
        <div class="container">
                <div class="row semi-header-row with-shadow">
                    <div class="col-sm-3 table-name">
                       <h5 class="slim-font">List of Registered Stores</h5> 
                    </div>
                    <div class="col-sm-1"></div>
                    <div class="col-sm-5 search-bar-div">
                        <form class="d-flex search-bar-parent" role="search">
                            <input class="form-control me-2 search-bar" id="search-bar" onkeyup="searchFunction()" type="search" placeholder="Type store name ... " aria-label="Search">
                                <button class="btn search-btn disabled" >Search</button>
                        </form>
                    </div>
                    <div class="col-sm-1"></div>
                    <div class="col-sm-2 d-flex justify-content-center">
                        <a href="{{route('store.create')}}" class="btn custom-pill-register">Register a Store</a>
                    </div>
                </div>
                ...
                <div class="table-responsive with-shadow justify-content-end">
                    ...
                    @if($stores->isEmpty())
                        <div class="d-flex justify-content-center">
                            <div class="col-sm-6 form-name no-record-info">
                                <h4>NO RECORDS FOUND</h4>
                                <p class="fw-lighter">Register a store to utilize records view page.</p>
                            </div>
                        </div>
                    @else
                        <thead>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                <div class="d-flex justify-content-center">
                                    <form method="POST" action="{{route('store.edit', $store->id)}}" accept-charset="UTF-8">
                                        {{csrf_field()}}   
                                        <button type="Submit" class="btn custom-pill-edit">Edit</button>
                                    </form>

                                    <!-- Button trigger modal -->
                                        <button type="button" class="btn custom-pill-delete" data-bs-toggle="modal" data-bs-target="#deleteModal{{$store->id}}">Delete</button>
                                </div>
                                </td>
                            </tr>
                            ...
                            <div class="modal fade" id="deleteModal{{$store->id}}" tabindex="-1" aria-labelledby="deleteModalLabel{{$store->id}}" aria-hidden="true">
                                <div class="modal-dialog">
                                    ...
                                    <div class="modal-body">
                                        Are you sure you want to delete this store record? This will delete all registered employees under the store as well. <br><br>This action can not be undone.
                                    </div>
                                    <div class="modal-footer">
                                        <form method="POST" action="{{route('store.delete', $store->id)}}" accept-charset="UTF-8">
                                        {{csrf_field()}}   
